...
function App() {
  ...
  const calc = () =>{
    //if last character is an operator we need to make sure we do nothing
    if(isOperator(et.charAt(et.length -1))) return;
    //clean expression so that we can make sure two operators in a row use last operator

    const parts = et.split(" ");
    const new_parts = [];
    for(let i=parts.length -1; i >= 0; i--){
        if(["*", "/", "+"].includes(parts[i]) && isOperator(parts[i - 1])){
         new_parts.unshift(parts[i]);
         let j=0;
         let k = i -1;
    ...
    const newExpression = new_parts.join(" ");
